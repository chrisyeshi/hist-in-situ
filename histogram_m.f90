module histogram_m
  ...
  character, dimension(limit,3) :: vars_hist*15
    ...
    if(myid .eq. 0) then
      ...
      do
        ...
        if (ndim_hist(nHistConfigs) > 0) then
          read(io_inp, *) vars_hist(nHistConfigs, 1), mins_hist(nHistConfigs, 1), maxs_hist(nHistConfigs, 1)
        endif
        if (ndim_hist(nHistConfigs) > 1) then
          read(io_inp, *) vars_hist(nHistConfigs, 2), mins_hist(nHistConfigs, 2), maxs_hist(nHistConfigs, 2)
        endif
        if (ndim_hist(nHistConfigs) > 2) then
          read(io_inp, *) vars_hist(nHistConfigs, 3), mins_hist(nHistConfigs, 3), maxs_hist(nHistConfigs, 3)
        endif
      enddo
    endif
  ...
  subroutine print_histogram_configs( io )
    ...
    do i=1,nHistConfigs
      ...
      write(io, *) "var1:", vars_hist(i, 1), mins_hist(i, 1), maxs_hist(i, 1)
      write(io, *) "var2:", vars_hist(i, 2), mins_hist(i, 2), maxs_hist(i, 2)
      write(io, *) "var3:", vars_hist(i, 3), mins_hist(i, 3), maxs_hist(i, 3)
  ...
  subroutine generate_and_output_histogram( io, iConfig )
    ...
    enddo
    call generate_and_output_histogram_nd(io, nDim, data, mins, maxs, nbin, iConfig)
    deallocate(data)
  ...
  subroutine generate_and_output_histogram_nd( io, nDim, data, mins, maxs, nbin, iConfig )
